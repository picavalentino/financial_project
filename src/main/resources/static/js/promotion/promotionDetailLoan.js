...
// 서버에서 제공된 한글 값
const selectedMethod = document.getElementById("selectedRepaymentMethod").value;

// 한글 값과 select의 value 값 매핑
const methodMapping = {
    "원금균등상환": "equalPrincipal",
    "원리금균등상환": "equalPrincipalAndInterest",
    "원금만기일시상환": "lumpSum",
};

// 셀렉트 박스 설정
const repaymentSelect = document.getElementById("repaymentMethod");

if (selectedMethod) {
    // 매핑 테이블을 통해 한글 값을 영어 값으로 변환
    const mappedValue = methodMapping[selectedMethod];

    if (mappedValue) {
        // repaymentMethod에서 변환된 값에 해당하는 옵션 선택
        for (let option of repaymentSelect.options) {
...
<!--            console.log('Collected Data:', data);-->
    ...
    // 서버로 POST 요청 전송
    fetch('/promotion/cal/loan/update', {
        method: 'POST',
        headers: {
