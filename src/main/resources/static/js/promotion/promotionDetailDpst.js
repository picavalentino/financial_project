...
window.addEventListener('DOMContentLoaded', () => {
        const button = document.querySelector('.btn-int-cal'); // 버튼 선택
         if (button) {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // 수정일자 (dsgnUpdateAt) 필드만 오늘 날짜로 설정
    ...
    async function loadProductList(productCode = '', productName = '') {
        try {
            const response = await fetch(`/promotion/cal/dpstProductList?prodCd=${encodeURIComponent(productCode)}&prodNm=${encodeURIComponent(productName)}`);
    ...
    function populateCustomerForm(user) {
        ...
        document.getElementById('custCreateAt').value = user.custCreateAt || ''; // 수정일자
...
function initializeChart() {
    ...
    resultChart = new Chart(ctx, {
        type: 'bar',
        data: {
            labels: ['총예치금액', '세전이자', '세전수령액', '이자과세금', '세후수령액'],
            datasets: [{
                label: '계산 결과',
...
function handleInterestCalculation() {
    ...
    const requestData = {
        depositAmt: parseFloat(savgCircleAmt.replace(/,/g, '')),
        goalPeriod: parseInt(savgGoalPrd),
        aplyRate: parseFloat(savgAplyRate),
        taxRate: parseFloat(document.getElementById('prodIntTaxRate').value),
        startDate: document.getElementById('savgStrtDt').value,
    };

    // 서버 요청
    fetch('/promotion/cal/dpstCalculate', {
        method: 'POST',
...
function updateCalculationResults(data) {
    document.getElementById('savgTotDpstAmt').value = data.totalDepositAmt.toLocaleString();
    document.getElementById('savgTotDpstInt').value = data.totalInterest.toLocaleString();
    document.getElementById('savgTotRcveAmt').value = data.totalPreTax.toLocaleString();
    document.getElementById('savgIntTaxAmt').value = data.taxAmount.toLocaleString();
    document.getElementById('savgAtxRcveAmt').value = data.totalAfterTax.toLocaleString();

    // 차트 데이터 업데이트
    if (resultChart && resultChart.data && resultChart.data.datasets) {
        resultChart.data.datasets[0].data = [
            data.totalDepositAmt,
            data.totalInterest,
            data.totalPreTax,
            data.taxAmount,
            data.totalAfterTax
    ...
    data.detail.forEach((detail) => {
        const row = `
            <tr>
                ...
                <td>${detail.interest.toLocaleString()}</td>
                <td>${detail.taxAmount.toLocaleString()}</td>
                <td>${detail.afterTaxAmount.toLocaleString()}</td>
            </tr>`;
    ...
    detailTableBody.insertAdjacentHTML('beforeend', row);
        ...
        console.error('data.detail의 타입이 예상과 다릅니다:', data.detail);
    }
    }
...
<!--            }-->
    ...
    const savingsSaveDto = {
        dsgnSno: document.getElementById('dsgnSn').value, // 설계번호
        prodSn: prodSn, // 상품번호
        custId: custId, // 고객ID
        userId: userId, // 담당자ID
...
<!--            console.log('Collected Data:', data);-->
    ...
    // 서버로 POST 요청 전송
    fetch('/promotion/cal/dpst/update', {
        method: 'POST',
        headers: {
