...
customerContainer.classList.remove('visible');
    ...
    async function loadProductList(productCode = '', productName = '') {
        try {
            const response = await fetch(`/promotion/cal/productList?prodCd=${encodeURIComponent(productCode)}&prodNm=${encodeURIComponent(productName)}`);
    ...
    function populateCustomerForm(user) {
        ...
        document.getElementById('custCreateAt').value = user.custCreateAt || ''; // 설계일자
...
<!--            }-->
    ...
    const savingsSaveDto = {
        ...
        console.log(prodDsTyCd);
<!--            // 데이터 확인 로그-->
<!--            console.log('Collected Data:', data);-->

    // 서버로 POST 요청 전송
    fetch('/promotion/cal/savg/update', {
        method: 'POST',
        headers: {
